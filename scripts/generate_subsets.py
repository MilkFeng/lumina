import json
import os
import subprocess
import urllib.request
import ssl

# ================= 配置区域 =================

# ARB 文件路径 (UI 文案)
ARB_FILE = '../lib/l10n/app_zh.arb'

# 字体文件夹路径
FONT_DIR = '../assets/fonts/'

# 字体下载地址
FONT_DOWNLOAD_BASE_URL = 'https://github.com/adobe-fonts/source-han-serif/releases/latest/download/09_SourceHanSerifSC.zip'
FONTS_CONFIG = {}

CHAR_SOURCES = [
    {
        "name": "xiandai_changyong.txt", # 现代汉语常用字表 (老3500)
        "url": "https://raw.githubusercontent.com/NightFurySL2001/cjktables/master/china/standard/xiandai_changyong.txt"
    },
    {
        "name": "yiwu_jiaoyu.txt", # 义务教育语文课程常用字表 (新3500)
        "url": "https://raw.githubusercontent.com/NightFurySL2001/cjktables/master/china/standard/yiwu_jiaoyu.txt"
    },
    {
        "name": "gb_t_2312.txt", # GB/T 2312-80 (6763字)
        "url": "https://raw.githubusercontent.com/NightFurySL2001/cjktables/master/china/encoding/gb_t_2312.txt"
    },
    {
        "name": "tongyong_guifan.txt", # 通用规范汉字表 (8105字)
        "url": "https://raw.githubusercontent.com/NightFurySL2001/cjktables/master/china/standard/tongyong_guifan.txt"
    },
    {
        "name": "biaozhun_dianma.txt", # 《标准电码本（修订本）》(1983年)含编号
        "url": "https://raw.githubusercontent.com/zispace/hanzi-chars-ext/master/%E8%A7%84%E8%8C%83%E5%AD%97%E8%A1%A8%E8%A1%A5%E5%85%85/%E3%80%8A%E6%A0%87%E5%87%86%E7%94%B5%E7%A0%81%E6%9C%AC%EF%BC%88%E4%BF%AE%E8%AE%A2%E6%9C%AC%EF%BC%89%E3%80%8B%EF%BC%881983%E5%B9%B4%EF%BC%89%E5%90%AB%E7%BC%96%E5%8F%B7.txt"
    },
    {
        "name": "gbk.txt", # GBK-1995 字符集
        "url": "https://raw.githubusercontent.com/zispace/hanzi-chars/master/data-charset/GBK-1995.txt"
    }
]

# 你的字体家族名称
FONT_FAMILY_NAME = "AppSerif"
FONT_FAMILY_CONTENT_NAME = "AppSerifContent"

# ===========================================

def get_combined_chars():
    """下载并合并所有指定的汉字字符集"""
    combined_chars = set()
    
    # 忽略 SSL 证书验证 (防止某些网络环境下报错)
    ssl_context = ssl._create_unverified_context()

    print(f"🌐 准备获取扩展字符集 (共 {len(CHAR_SOURCES)} 个来源)...")

    for source in CHAR_SOURCES:
        filename = source["name"]
        url = source["url"]
        
        # 1. 如果本地没有，先下载
        if not os.path.exists(filename):
            print(f"   ⬇️ 正在下载: {filename} ...")
            try:
                # 使用 urllib 下载，传入 ssl context
                with urllib.request.urlopen(url, context=ssl_context) as response, open(filename, 'wb') as out_file:
                    out_file.write(response.read())
                print(f"      ✅ 下载成功")
            except Exception as e:
                print(f"      ❌ 下载失败 [{filename}]: {e}")
                continue
        else:
            print(f"   📂 读取本地文件: {filename}")

        # 2. 读取并解析字符
        try:
            with open(filename, 'r', encoding='utf-8') as f:
                content = f.read()
                # 简单解析：过滤掉 # 开头的注释行（如果有），然后提取所有非空白字符
                count = 0
                for line in content.splitlines():
                    if line.strip().startswith('#'): continue
                    for char in line:
                        if char.strip(): # 只要不是空格/换行
                            combined_chars.add(char)
                            count += 1
                # print(f"      + 添加了 {count} 个字符")
        except Exception as e:
            print(f"      ❌ 读取失败: {e}")

    print(f"📊 字符集处理完毕，去重后共计: {len(combined_chars)} 个汉字")
    return combined_chars

def download_and_extract_font():
    """下载并解压字体文件"""
    zip_path = 'source_han_serif.zip'
    
    if not os.path.exists(zip_path):
        print(f"⬇️ 正在下载字体文件: {FONT_DOWNLOAD_BASE_URL} ...")
        try:
            urllib.request.urlretrieve(FONT_DOWNLOAD_BASE_URL, zip_path)
            print(f"   ✅ 字体下载成功")
        except Exception as e:
            print(f"   ❌ 字体下载失败: {e}")
            return False
    else:
        print(f"📂 字体压缩包已存在: {zip_path}")

    # 解压到 FONT_DIR
    if not os.path.exists('./fonts'):
        try:
            import zipfile
            with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                zip_ref.extractall('./fonts')
            print(f"   ✅ 字体解压成功到: './fonts'")
        except Exception as e:
            print(f"   ❌ 字体解压失败: {e}")
            return False
    
    # 更新 FONTS_CONFIG
    for file in os.listdir('./fonts/OTF/SimplifiedChinese'):
        if file.endswith('.otf'):
            weight = None
            if 'ExtraLight' in file:
                weight = 200
            elif 'Light' in file:
                weight = 300
            elif 'Regular' in file:
                weight = 400
            elif 'Medium' in file:
                weight = 500
            elif 'SemiBold' in file:
                weight = 600
            elif 'Bold' in file:
                weight = 700
            elif 'Heavy' in file:
                weight = 900

            path = os.path.join('./fonts/OTF/SimplifiedChinese', file)
            
            if weight is not None:
                FONTS_CONFIG[path] = weight

def main():
    final_chars = set()

    # 0. 下载原始字体文件并解压
    print("🚀 开始生成字体子集...")
    download_and_extract_font()

    # 1. 提取 ARB 字符 (UI)
    print(f"📖 正在读取 ARB 文件: {ARB_FILE} ...")
    if os.path.exists(ARB_FILE):
        with open(ARB_FILE, 'r', encoding='utf-8') as f:
            data = json.load(f)
        for key, value in data.items():
            if key.startswith('@'): continue
            final_chars.update(list(value))
        print(f"   已提取 ARB 字符，当前总数: {len(final_chars)}")
    else:
        print(f"⚠️ 警告: 找不到 ARB 文件 {ARB_FILE}，将跳过 UI 字符提取。")

    # 2. 添加 ASCII (英文/数字/符号)
    ascii_chars = set(chr(i) for i in range(0x0020, 0x007F))
    final_chars.update(ascii_chars)
    
    # 3. 添加特殊字符
    special_chars = """
✐✎✏✑✒✍✉✁✂✃✄✆✉☎☏☑✓✔√☐☒✗✘ㄨ✕✖✖☢☠☣✈★☆✡囍㍿☯☰☲☱☴☵☶☳☷☜☞☝✍☚☛☟✌♤♧♡♢♠♣♥♦☀☁☂❄☃♨웃유❖☽☾☪✿♂♀✪✯☭➳卍卐√×■◆●○◐◑✙☺☻❀⚘♔♕♖♗♘♙♚♛♜♝♞♟♧♡♂♀♠♣♥❤☜☞☎☏⊙◎☺☻☼▧▨♨◐◑↔↕▪▒◊◦▣▤▥▦▩◘◈◇♬♪♩♭♪の★☆→あぃ￡Ю〓§♤♥▶¤✲❈✿✲❈➹☀☂☁【】┱┲❣✚✪✣✤✥✦❉❥❦❧❃❂❁❀✄☪☣☢☠☭ღ▶▷◀◁☀☁☂☃☄★☆☇☈⊙☊☋☌☍ⓛⓞⓥⓔ╬『』∴☀♫♬♩♭♪☆∷﹌の★◎▶☺☻►◄▧▨♨◐◑↔↕↘▀▄█▌◦☼♪の☆→♧ぃ￡❤▒▬♦◊◦♠♣▣۰•❤•۰►◄▧▨♨◐◑↔↕▪▫☼♦⊙●○①⊕◎Θ⊙¤㊣★☆♀◆◇◣◢◥▲▼△▽⊿◤◥✐✌✍✡✓✔✕✖♂♀♥♡☜☞☎☏⊙◎☺☻►◄▧▨♨◐◑↔↕♥♡▪▫☼♦▀▄█▌▐░▒▬♦◊◘◙◦☼♠♣▣▤▥▦▩◘◙◈♫♬♪♩♭♪✄☪☣☢☠♯♩♪♫♬♭♮☎☏☪♈ºº₪¤큐«»™♂✿♥　◕‿-｡　｡◕‿◕｡
✽✾✿❁❃❋❀⚘☑✓✔√☐☒✗✘✕✖✖⋆✢✣✤✥❋✦✧✩✰✪✫✬✭✮✯❂✡★✱✲✳✴✵✶✷✸✹✺✻✼❄❅❆❇❈❉❊†☨✞✝☥☦☓☩☯☧☬☸✡♁✙♆。，、＇：∶；?‘’“”〝〞ˆˇ﹕︰﹔﹖﹑•¨….¸;！´？！～—ˉ｜‖＂〃｀@﹫¡¿﹏﹋﹌︴々﹟#﹩$﹠&﹪%*﹡﹢﹦﹤‐￣¯―﹨ˆ˜﹍﹎+=<＿_-\ˇ~﹉﹊（）〈〉‹›﹛﹜『』〖〗［］《》〔〕{}「」【】︵︷︿︹︽_﹁﹃︻︶︸﹀︺︾ˉ﹂﹄︼☯♠♣♧♡♥❤❥❣♂♀✲☀☼☾☽◐◑☺☻☎☏✿❀№↑↓←→√×÷★℃℉°◆◇⊙■□△▽¿½☯✡㍿卍卐♂♀✚〓㎡♪♫♩♬㊚㊛囍㊒㊖Φ♀♂‖$@*&#※卍卐Ψ♫♬♭♩♪♯♮⌒¶∮‖€￡¥$☩☨☦✞✛✜✝✙✠✚†‡◉○◌◍◎●◐◑◒◓◔◕◖◗❂☢⊗⊙◘◙◍ℂℍℕℙℚℝℤℬℰℯℱℊℋℎℐℒℓℳℴ℘ℛℭ℮ℌℑℜℨ♪♫♩♬♭♮♯°øⒶ☮✌☪✡☭✯卐✐✎✏✑✒✍✉✁✂✃✄✆✉
☎☏➟➡➢➣➤➥➦➧➨➚➘➙➛➜➝➞➸♐➲➳⏎➴➵➶➷➸➹➺➻➼➽←↑→↓↔↕↖↗↘↙↚↛↜↝↞↟↠↡↢↣↤↥↦↧↨➫➬➩➪➭➮➯➱↩↪↫↬↭↮↯↰↱↲↳↴↵↶↷↸↹↺↻↼↽↾↿⇀⇁⇂⇃⇄⇅⇆⇇⇈⇉⇊⇋⇌⇍⇎⇏⇐⇑⇒⇓⇔⇕⇖⇗⇘⇙⇚⇛⇜⇝⇞⇟⇠⇡⇢⇣⇤⇥⇦⇧⇨⇩⇪
ⅠⅡⅢⅣⅤⅥⅦⅧⅨⅩⅪⅫⅬⅭⅮⅯⅰⅱⅲⅳⅴⅵⅶⅷⅸⅹⅺⅻⅼⅽⅾⅿ┌┍┎┏┐┑┒┓└┕┖┗┘┙┚┛├┝┞┟┠┡┢┣┤┥┦┧┨┩┪┫┬┭┮┯┰┱┲┳┴┵┶┷┸┹┺┻┼┽┾┿╀╁╂╃╄╅╆╇╈╉╊╋╌╍╎╏═║╒╓╔╕╖╗╘╙╚╛╜╝╞╟╠╡╢╣╤╥╦╧╨╩╪╫╬◤◥◄►▶◀◣◢▲▼◥▸◂▴▾△▽▷◁⊿▻◅▵▿▹◃❏❐❑❒▀▐░▒▓▔▕■□▢▣▤▥▦▧▨▩▪▫▬▭▮▯㋀㋁㋂㋃㋄㋅㋆㋇㋈㋉㋊㋋㏠㏡㏢㏣㏤㏥㏦㏧㏨㏩㏪㏫㏬㏭㏮㏯㏰㏱㏲㏳㏴㏵㏶㏷㏸㏹㏺㏻㏼㏽㏾㍙㍚㍛㍜㍝㍞㍟㍠㍡㍢㍣㍤㍥㍦㍧㍨㍩㍪㍫㍬㍭㍮㍯㍰㍘☰☲☱☴☵☶☳☷
①②③④⑤⑥⑦⑧⑨⑩⑪⑫⑬⑭⑮⑯⑰⑱⑲⑳⓪⓿❶❷❸❹❺❻❼❽❾❿⓫⓬⓭⓮⓯⓰⓱⓲⓳⓴⓵⓶⓷⓸⓹⓺⓻⓼⓽⓾㊀㊁㊂㊃㊄㊅㊆㊇㊈㊉
㈠㈡㈢㈣㈤㈥㈦㈧㈨㈩⑴⑵⑶⑷⑸⑹⑺⑻⑼⑽⑾⑿⒀⒁⒂⒃⒄⒅⒆⒇
⒈⒉⒊⒋⒌⒍⒎⒏⒐⒑⒒⒓⒔⒕⒖⒗⒘⒙⒚⒛ⅠⅡⅢⅣⅤⅥⅦⅧⅨⅩⅪⅫⅰⅱⅲⅳⅴⅵⅶⅷⅸⅹ
ⒶⒷⒸⒹⒺⒻⒼⒽⒾⒿⓀⓁⓂⓃⓄⓅⓆⓇⓈⓉⓊⓋⓌⓍⓎⓏⓐⓑⓒⓓⓔⓕⓖⓗⓘⓙⓚⓛⓜⓝⓞⓟⓠⓡⓢⓣⓤⓥⓦⓧⓨⓩ⒜⒝⒞⒟⒠⒡⒢⒣⒤⒥⒦⒧⒨⒩⒪⒫⒬⒭⒮⒯⒰⒱⒲⒳⒴⒵
﹢﹣×÷±+-*/^=≌∽≦≧≒﹤﹥≈≡≠≤≥≮≯∷∶∝∞∧∨∑∏∪∩∈∵∴⊥∥∠⌒⊙√∛∜∟⊿㏒㏑%‰⅟½⅓⅕⅙⅐⅛⅑⅒⅔¾⅖⅗⅘⅚⅜⅝⅞≂≃≄≅≆≇≉≊≋≍≎≏≐≑≓≔≕≖≗≘≙≚≛≜≝≞≟≢≣≨≩⊰⊱⋛⋚∫∮∬∭∯∰∱∲∳℅øπ∀∁∂∃∄∅∆∇∉∊∋∌∍∎∐−∓∔∕∖∗∘∙∡∢∣∤∦∸∹∺∻∼∾∿≀≁≪≫≬≭≰≱≲≳≴≵≶≷≸≹≺≻≼≽≾≿⊀⊁⊂⊃⊄⊅⊆⊇⊈⊉⊊⊋⊌⊍⊎⊏⊐⊑⊒⊓⊔⊕⊖⊗⊘⊚⊛⊜⊝⊞⊟⊠⊡⊢⊣⊤⊦⊧⊨⊩⊪⊫⊬⊭⊮⊯⊲⊳⊴⊵⊶⊷⊸⊹⊺⊻⊼⊽⊾⋀⋁⋂⋃⋄⋅⋆⋇⋈⋉⋊⋋⋌⋍⋎⋏⋐⋑⋒⋓⋔⋕⋖⋗⋘⋙⋜⋝⋞⋟⋠⋡⋢⋣⋤⋥⋦⋧⋨⋩⋪⋫⋬⋭⋮⋯⋰⋱⋲⋳⋴⋵⋶⋷⋸⋹⋺⋻⋼⋽⋾⋿ⅠⅡⅢⅣⅤⅥⅦⅧⅨⅩⅪⅫⅬⅭⅮⅯↁↂↃↅↆↇↈ↉↊↋■□▢▣▤▥▦▧▨▩▪▫▬▭▮▯▰▱▲△▴▵▶▷▸▹►▻▼▽▾▿◀◁◂◃◄◅◆◇◈◉◊○◌◍◎●◐◑◒◓◔◕◖◗◘◙◚◛◜◝◞◟◠◡◢◣◤◥◦◧◨◩◪◫◬◭◮◯◰◱◲◳◴◵◶◷◸◹◺◿◻◼◽◾⏢⏥⌓⌔⌖
⁰ ¹ ² ³ ⁴ ⁵ ⁶ ⁷ ⁸ ⁹ ⁺ ⁻ ⁼ ⁽ ⁾ ⁿ ₀ ₁ ₂ ₃ ₄ ₅ ₆ ₇ ₈ ₉ ₊ ₋ ₌ ₍ ₎ ₐ ₑ ₒ ₓ ₔ ₕ ₖ ₗ ₘ ₙ ₚ ₛ ₜ
♥❣ღ♠♡♤❤❥
。，、＇：∶；?‘’“”〝〞ˆˇ﹕︰﹔﹖﹑•¨….¸;！´？！～—ˉ｜‖＂〃｀@﹫¡¿﹏﹋﹌︴々﹟#﹩$﹠&﹪%*﹡﹢﹦﹤‐￣¯―﹨ˆ˜﹍﹎+=<＿_-\ˇ~﹉﹊（）〈〉‹›﹛﹜『』〖〗［］《》〔〕{}「」【】︵︷︿︹︽_﹁﹃︻︶︸﹀︺︾ˉ﹂﹄︼❝❞‐‑‒–―‖‗‘’‚‛“”„‟†‡•‣․‥…‧‰‱′″‴‵‶‷‸※‼‽‾‿⁀⁁⁂⁃⁄⁇⁈⁉⁊⁋⁌⁍⁎⁏⁐⁑⁒⁓⁔⁕⁖⁗⁘⁙⁚⁛⁜⁝⁞
°′″＄￥〒￠￡％＠℃℉﹩﹪‰﹫㎡㎥m²m³㎜㎟㎣㎝㎠㎤㍷㍸㍹㎞㎢㎦㏎㎚㎛㏕㎍㎎㎏㏄º○¤%$º¹²³㍺㎀㎁㎂㎃㎄㎅㎆㎇㎈㎉㎊㎋㎌㎐㎑㎒㎓㎔㎕㎖㎗㎘㎙㎧㎨㎩㎪㎫㎬㎭㎮㎯㎰㎱㎲㎳㎴㎵㎶㎷㎸㎹㎺㎻㎼㎽㎾㎿㏀㏁㏂㏃㏄㏅㏆㏇㏈㏉㏊㏋㏌㏍㏎㏏㏐㏑㏒㏓㏔㏕㏖㏗㏘㏙㏚㏛㏜㏝㏞㏟㍱㍲㍳㍴㍵㍶
↑↓←→↖↗↘↙↔↕➻➼➽➸➳➺➻➴➵➶➷➹▶►▷◁◀◄«»➩➪➫➬➭➮➯➱⏎➲➾➔➘➙➚➛➜➝➞➟➠➡➢➣➤➥➦➧➨↚↛↜↝↞↟↠↠↡↢↣↤↤↥↦↧↨⇄⇅⇆⇇⇈⇉⇊⇋⇌⇍⇎⇏⇐⇑⇒⇓⇔⇖⇗⇘⇙⇜↩↪↫↬↭↮↯↰↱↲↳↴↵↶↷↸↹☇☈↼↽↾↿⇀⇁⇂⇃⇞⇟⇠⇡⇢⇣⇤⇥⇦⇧⇨⇩⇪↺↻⇚⇛
€£Ұ₴$₰¢₤¥₳₲₪₵元₣₱฿¤₡₮₭₩ރ円₢₥₫₦zł﷼₠₧₯₨Kčर₹ƒ₸￠
āáǎàōóǒòēéěèīíǐìūúǔùǖǘǚǜüêɑńňɡㄅㄆㄇㄈㄉㄊㄋㄌㄍㄎㄏㄐㄑㄒㄓㄔㄕㄖㄗㄘㄙㄚㄛㄜㄝㄞㄟㄠㄡㄢㄣㄤㄥㄦㄧㄨㄩ
ぁあぃいぅうぇえぉおかがきぎくぐけげこごさざしじすずせぜそぞただちぢっつづてでとどなにぬねのはばぱひびぴふぶぷへべぺほぼぽまみむめもゃやゅゆょよらりるれろゎわゐゑをんゔゕゖァアィイゥウェエォオカガキギクグケゲコゴサザシジスズセゼソゾタダチヂッツヅテデトドナニヌネノハバパヒビピフブプヘベペホボポマミムメモャヤュユョヨラリルレロヮワヰヱヲンヴヵヶヷヸヹヺ・ーヽヾヿ゠ㇰㇱㇲㇳㇴㇵㇶㇷㇸㇹㇺㇻㇼㇽㇾㇿ
零壹贰叁肆伍陆柒捌玖拾佰仟万亿吉太拍艾分厘毫微卍卐卄巜弍弎弐朤氺曱甴囍兀々〆のぁ〡〢〣〤〥〦〧〨〩㊎㊍㊌㊋㊏㊚㊛㊐㊊㊣㊤㊥㊦㊧㊨㊒㊫㊑㊓㊔㊕㊖㊗㊘㊜㊝㊞㊟㊠㊡㊢㊩㊪㊬㊭㊮㊯㊰㊀㊁㊂㊃㊄㊅㊆㊇㊈㊉
─ ━│┃╌╍╎╏┄ ┅┆┇┈ ┉┊┋┌┍┎┏┐┑┒┓└ ┕┖┗ ┘┙┚┛├┝┞┟┠┡┢┣ ┤┥┦┧┨┩┪┫┬ ┭ ┮ ┯ ┰ ┱ ┲ ┳ ┴ ┵ ┶ ┷ ┸ ┹ ┺ ┻┼ ┽ ┾ ┿ ╀ ╁ ╂ ╃ ╄ ╅ ╆ ╇ ╈ ╉ ╊ ╋ ╪ ╫ ╬═║╒╓╔ ╕╖╗╘╙╚ ╛╜╝╞╟╠ ╡╢╣╤ ╥ ╦ ╧ ╨ ╩ ╳╔ ╗╝╚ ╬ ═ ╓ ╩ ┠ ┨┯ ┷┏ ┓┗ ┛┳ ⊥ ﹃ ﹄┌ ╮ ╭ ╯╰
♚♛♝♞♜♟♔♕♗♘♖♟
ΑΒΓΔΕΖΗΘΙΚΛΜΝΞΟΠΡΣΤΥΦΧΨΩαβγδεζνξοπρσηθικλμτυφχψω
АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдеёжзийклмнопрстуфхцчшщъыьэюя
〇
""".strip().replace("\n", "")
    final_chars.update(list(special_chars))

    # 3'. 添加全角字符 (防止遗漏)
    fullwidth_chars = set(chr(i) for i in range(0xFF01, 0xFF5E + 1))
    final_chars.update(fullwidth_chars)

    # 4. 提取常用字
    regular_chars = final_chars.copy()
    common_chars = get_combined_chars()
    if common_chars:
        regular_chars.update(common_chars)
        print(f"   已合并常用字，当前总数: {len(regular_chars)}")

    # 5. 生成临时字符集文件
    subset_text = "".join(sorted(final_chars))
    text_file = 'subset_temp.txt'
    with open(text_file, 'w', encoding='utf-8') as f:
        f.write(subset_text)

    regular_subset_text = "".join(sorted(regular_chars))
    regular_text_file = 'subset_regular_temp.txt'
    with open(regular_text_file, 'w', encoding='utf-8') as f:
        f.write(regular_subset_text)
    
    print(f"\n📝 最终字符集准备完毕，共包含 {len(regular_subset_text)} 个字符。")
    print("🚀 开始批量生成字体 (这可能需要几分钟)...\n")

    generated_results = []

    # 6. 循环处理每个字重
    for source_path, weight in FONTS_CONFIG.items():
        file_base_name = os.path.splitext(os.path.basename(source_path))[0]
        output_filename = f"{file_base_name}-Subset.ttf" # 换个后缀以示区分
        output_path = os.path.join(FONT_DIR, output_filename)

        if not os.path.exists(source_path):
            print(f"⚠️ 跳过: 找不到源文件 {source_path}")
            continue

        print(f"⏳ 正在处理: {source_path} (Weight: {weight})...")

        command = [
            'pyftsubset',
            source_path,
            f'--text-file={regular_text_file if weight == 400 else text_file}',
            f'--output-file={output_path}',
            '--layout-features=*', 
            '--obfuscate-names',
            '--no-hinting',
            '--desubroutinize'
        ]

        try:
            subprocess.run(command, check=True)
            
            old_size = os.path.getsize(source_path) / 1024 / 1024
            new_size = os.path.getsize(output_path) / 1024 / 1024 # 用 MB 显示比较直观
            
            print(f"   ✅ 成功! {old_size:.2f} MB -> {new_size:.2f} MB")
            
            generated_results.append({
                "asset": f"assets/fonts/{output_filename}",
                "weight": weight
            })
            
        except subprocess.CalledProcessError as e:
            print(f"   ❌ 处理失败: {e}")

    # 7. 清理
    if os.path.exists(text_file):
        os.remove(text_file)

    if os.path.exists(regular_text_file):
        os.remove(regular_text_file)

    # 8. 生成 Pubspec 配置
    generated_results.sort(key=lambda x: x['weight'])
    print("\n" + "="*50)

    print("🎉 全部完成！请更新你的 pubspec.yaml:")
    print("="*50)
    print(f"  fonts:")
    print(f"    - family: {FONT_FAMILY_NAME}")
    print(f"      fonts:")
    for item in generated_results:
        if (item['weight'] == 400):
            continue
        print(f"        - asset: {item['asset']}")
        print(f"          weight: {item['weight']}")

    print(f"    - family: {FONT_FAMILY_CONTENT_NAME}")
    print(f"      fonts:")
    for item in generated_results:
        if (item['weight'] != 400):
            continue
        print(f"        - asset: {item['asset']}")
        print(f"          weight: {item['weight']}")

    print("="*50)

if __name__ == '__main__':
    main()